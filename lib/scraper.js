...
module.exports = function (url) {
    ...
    return new Promise((resolve, reject) => {
      request
        ...
        .end((err, xres) => {
          ...
          const data = {}
          getDataFromSelectors(dom, selectors, data, xres.text)
  ...
  function getDomText (dom, selector, docText) {
    let text = ''
    ...
    if (/^\/\//.test(selector)) {
       const document = parse5.parse(docText);
       const xhtml = xmlser.serializeToString(document);
       const doc = new xmlDom().parseFromString(xhtml);
       const select = xpath.useNamespaces({'x': 'http://www.w3.org/1999/xhtml'});
       const nodes = select(selector, doc);
       if (nodes.length) {
         el = nodes[0]
       }
    } else {
    ...
    el = el || dom.querySelector(selector)
    if (el) {
  ...
  function getDataFromSelectors (dom, selectors, data, text) {
    _.each(selectors, (v, k) => {
      ...
      let t = getDomText(dom.window.document, v, text)
